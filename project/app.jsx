// Chord Progression Generator — main React app
// Renders inside <IOSDevice> from ios-frame.jsx
...
function App() {
  ...
  const [keyPc, setKeyPc] = React.useState(0); // C
  ...
  const scrollSounds = () => {
    ...
    if (max <= 0) return;
    // wrap when already at end
    if (el.scrollLeft >= max - 4) {
      el.scrollTo({ left: 0, behavior: 'smooth' });
    } else {
      el.scrollTo({ left: Math.min(max, el.scrollLeft + el.clientWidth * 0.7), behavior: 'smooth' });
    }
  };
  ...
  React.useEffect(() => {
    ...
    e.onBeat = () => setBeatTick((t) => t + 1);
    return () => {e.stop();e.dispose();};
  }, []);

  // Live-apply tempo / loop / metronome while playing
  React.useEffect(() => {engineRef.current.setBpm(t.tempo);}, [t.tempo]);
  React.useEffect(() => {engineRef.current.setLoop(t.loop);}, [t.loop]);
  React.useEffect(() => {engineRef.current.setMetronome(t.metronome && isPlaying);}, [t.metronome, isPlaying]);

  // Regenerate on any constraint change
  const regen = React.useCallback(() => {
    const p = generateProgression(keyPc, mode, length, { extension: t.extension });
    setProgression(p);
    setActiveIdx(-1);
    // If currently playing, swap the progression in immediately so the new
    // chords are heard without having to stop/start.
    if (engineRef.current && engineRef.current.playing) {
      engineRef.current.swapProgression(p);
    }
  }, [keyPc, mode, length, t.extension]);

  React.useEffect(() => {regen();}, [keyPc, mode, length, t.extension]);
  ...
  return (
    <div style={{
      ...
      right={<span style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: 12, color: 'oklch(0.55 0 0)' }}>{length} chords</span>}>
      ...
      {/* Randomize — primary action, matching button vocabulary */}
      ...
      {/* Extensions — segmented selector below progression */}
      <div style={{
        display: 'grid',
        ...
        {[
          ...
          <button key={opt.value}
            onClick={() => setTweak('extension', opt.value)}
            style={{
              padding: '7px 0',
              background: t.extension === opt.value ? accent.color : 'transparent',
              color: t.extension === opt.value ? '#0c0c0d' : 'oklch(0.75 0 0)',
              border: 'none',
              borderRadius: 9,
              fontFamily: 'inherit',
              fontSize: 12,
              fontWeight: t.extension === opt.value ? 600 : 500,
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div ref={soundScrollerRef} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 6, overflowX: 'auto', scrollbarWidth: 'none', minWidth: 0, scrollBehavior: 'smooth' }}
          className="hide-scroll">
            {Object.entries(SOUNDS).map(([k, s]) =>
            <button key={k}
            onClick={() => setTweak('sound', k)}
            style={{
              ...soundChipStyle(t.sound === k, accent),
              flex: '0 0 auto'
            }}>
                {s.label}
              </button>
        ...
        {/* Row 2: Loop + Metronome + Tempo + BPM pulse */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingTop: 4, borderTop: '1px solid oklch(0.24 0 0)' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <TempoMeter value={t.tempo} onChange={(v) => setTweak('tempo', v)}
            accent={accent} isPlaying={isPlaying} metroOn={t.metronome} beatTick={beatTick} />
          </div>
        </div>
      </div>
      ...
      <TweaksPanel title="Tweaks">
        ...
        <div style={{ display: 'flex', gap: 8, padding: '4px 0 12px' }}>
          ...
          style={{
            width: 28, height: 28, borderRadius: 14,
            background: v.color, border: t.accent === k ? '2px solid #fff' : '2px solid transparent',
            cursor: 'pointer', padding: 0
          }} aria-label={k} />
          )}
        </div>
        <TweakRadio label="Notation" value={t.notation}
...
function soundChipStyle(active, accent) {
  return {
...
function Stepper({ value, min, max, onChange, accent }) {
  ...
  return (
    <div style={{
      ...
      <StepBtn onClick={dec} disabled={value <= min}>−</StepBtn>
      ...
      <StepBtn onClick={inc} disabled={value >= max}>+</StepBtn>
    </div>);
...
function StepBtn({ children, onClick, disabled }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      border: 'none',
      color: disabled ? 'oklch(0.35 0 0)' : 'oklch(0.9 0 0)',
...
function ProgressionGrid({ progression, activeIdx, onTap, notation, accent }) {
  ...
  // Compute ONE font size that fits the LONGEST chord name in the row — every
  // card then uses the same size so the type feels rhythmically consistent.
  ...
  // Usable horizontal space: 402px device - 44px app padding = 358px
  const gridW = 358;
  const gap   = 8;
  const cardW = (gridW - (cols - 1) * gap) / cols;
  const innerPad = 12;
  // JetBrains Mono with -0.03em letter-spacing ≈ 0.58em per char
  ...
  return (
    <div style={{
      ...
      {progression.map((c, i) =>
      <ChordCard key={i} chord={c} active={i === activeIdx}
...
function TempoMeter({ value, onChange, accent, isPlaying, metroOn, beatTick }) {
  // CSS pulse animation duration = beat duration
  const beatSec = 60 / value;
  const pulseKey = `${value}-${isPlaying}`; // restart animation when tempo changes
  // When metronome is on AND playing, snap visual to actual beat callbacks
  const flash = metroOn && isPlaying ? beatTick : null;
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <input type="range" min={40} max={200} value={value}
      ...
      <PulseDot beatSec={beatSec} accent={accent} active={isPlaying} flashKey={flash} pulseKey={pulseKey} />
    </div>);

}

function PulseDot({ beatSec, accent, active, flashKey, pulseKey }) {
  // when metronome flash is provided, use a quick scale flash per beat
  if (flashKey !== null && flashKey !== undefined) {
    return <FlashDot accent={accent} flashKey={flashKey} />;
  }
  return (
...
function FlashDot({ accent, flashKey }) {
  return (
...
function IconToggle({ children, label, active, onClick, accent }) {
  return (
    <button onClick={onClick} title={label} aria-pressed={active} style={{
      ...
      flex: '0 0 auto',
      boxShadow: active ? `0 0 0 3px ${accent.glow}` : 'none'
    }}>
      {children}
    </button>);
    ...
    <div style={{
      ...
      <div style={{ width: W * scale, height: H * scale }}>
        <div style={{
          ...
          <IOSDevice dark={true} width={W} height={H}>
            <App />
          </IOSDevice>
        </div>
      </div>
